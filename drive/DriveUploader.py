...
class DriveUploader:
    ...
    def create_folder(self, folder_name, parent_id):
        ...
        existing_id     =  self.check_exists(folder_name, parent_id, is_folder=True)
        if existing_id:
    ...
    def upload_file(self, file_path, parent_id):
        ...
        existing_id      = self.check_exists(file_name, parent_id, is_folder=False)
        if existing_id:
    ...
    def upload_asset_folder(self):
        ...
        print(f"Starting upload of: {DOWNLOAD_ASSET_LOCATION}")
        if not os.path.exists(DOWNLOAD_ASSET_LOCATION):
            print(f"Error: {DOWNLOAD_ASSET_LOCATION} does not exist!")
            return
        # Create main "Asset" folder in Google Drive (or use existing)
        main_folder_name        = os.path.basename(DOWNLOAD_ASSET_LOCATION)
        main_folder_id          = self.create_folder(main_folder_name, GOOGLE_DRIVE_PARENT_FOLDER_ID)
        # Upload all contents recursively
        self.upload_folder(DOWNLOAD_ASSET_LOCATION, main_folder_id)
